    ...
    def post(self, command):
        if command == "setbyid":
            ...
            if id == -1:
                ...
                req = "INSERT INTO Character (" + str_att + \
                    ") VALUES(" + str_values + ");"
                ...
                for i in range(len(att)):
                    str_att = str_att + str(att[i]) + "=?,"
                str_att = str_att[:-1]
                req = "UPDATE Character SET " + \
                    str_att + " WHERE id="+str(id)+";"
                dao.execute(req, values)
            return {"code": 200}
        elif command == "getonebyid":
            result = dao.select(
                "SELECT * FROM Character WHERE id=(?);", [request.json["id"]])
            return {"code": 200, "data": result}
        elif command == "getall":
            result = dao.select("SELECT * FROM Character")
            return {"code": 200, "data": result}
        elif command == "delete":
            dao.execute("DELETE FROM Character WHERE id=(?);",
                        [request.json["id"]])
            return {"code": 200}
        return {"code": "404"}
